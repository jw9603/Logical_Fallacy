...
def load_model_electra():
    ...
    model = AutoModelForSequenceClassification.from_pretrained('google/electra-base-discriminator', num_labels=15, ignore_mismatched_sizes=True)
    
    return tokenizer_hf, model

def load_model_deberta():
    tokenizer_hf = AutoTokenizer.from_pretrained('microsoft/deberta-base')
    model = AutoModelForSequenceClassification.from_pretrained('microsoft/deberta-base', num_labels=15, ignore_mismatched_sizes=True)
...
if __name__ =='__main__':
    ...
    data_path = './data/propaganda/propaganda_all2.json' # Combine the training, validation, and test datasets.

    import torch
    torch.set_printoptions(threshold=1000000000000000000)
    
    with open('./result/propaganda/roberta_cg2_query_lr_1e5_ml512_ep40_bs_128_result_1time_15class.txt','w') as output_file:
        import sys
        ...
        dataset = load_dataset_cg(data_path)
