...
def load_dataset_cv(path, fold_index, n_splits=5):
    try:
    ...
    # Combine train and dev data to create folds
    all_train_data = json_data['train'] + json_data['dev'] + json_data['test']
    
    # Perform stratified split based on labels
    skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)
    for fold, (train_index, dev_index) in enumerate(skf.split(all_train_data, [sample[1] for sample in all_train_data])):
        if fold == fold_index:
            for idx in train_index:
                sample = all_train_data[idx]
                train_texts.append(sample[0])
            ...
            for idx in dev_index:
                ...
                if sample[1] == 'appeal to emotion':
                    ...
                    dev_labels.append(4)
            break  # We only need the specific fold
    
    train_data = {'label': train_labels, 'text': train_texts}
    dev_data = {'label': dev_labels, 'text': dev_texts}
    test_data = {'label': [sample[1] for sample in json_data['test']], 'text': [sample[0] for sample in json_data['test']]}
    
    final_data = DatasetDict({'train': Dataset.from_dict(train_data), 'dev': Dataset.from_dict(dev_data), 'test': Dataset.from_dict(test_data)})
...
def compute_metrics(eval_preds):
    ...
    # print('confidences_score###########',np.max(softmax_logits,axis=-1)[0])
...
if __name__ =='__main__':
    ...
    with open('./result/roberta/Argotario/Roberta_query_specific_v3_result_1time_5class.txt','w') as output_file:
        import sys
        ...
        for fold_index in range(n_splits):
            ...
            dataset = load_dataset_cv(data_path, fold_index)
            print(dataset)
            tknz, mdl = load_model()
        ...
        # Print true labels, predicted labels, and confidence scores
        for true_label, predicted_label, confidence_score in zip(test_labels, predicted_labels, confidence_scores):
            print(f"True Label: {true_label}, Predicted Label: {predicted_label}, Confidence Score: {confidence_score}")
        assert -1 == 0
